...
import { UpcomingCollection } from '@/types/atmosferico';
...
interface AtmosfericoDueDatesModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const AtmosfericoDueDatesModal: React.FC<AtmosfericoDueDatesModalProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  const fetchUpcoming = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      const data = await atmosfericoWebService.getUpcomingCollections();
      setCollections(data);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Error al obtener próximos cobros.';
      setError(msg);
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">
                Próximos Cobros Atmosféricos
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Servicios atmosféricos pendientes de cobro ordenados por fecha estimada.
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <Button variant="outline" size="sm" onClick={fetchUpcoming} disabled={isLoading}>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {isLoading ? (
            <div className="py-12 flex flex-col items-center justify-center space-y-2">
              ...
              <span className="text-xs text-slate-400 font-medium">Cargando cobros pendientes...</span>
            </div>
            ...
            <div className="py-12 flex flex-col items-center justify-center space-y-2 text-center">
              ...
              <p className="text-xs font-semibold text-slate-600 dark:text-slate-400">Sin cobros pendientes</p>
              <p className="text-[11px] text-slate-400">Todos los servicios atmosféricos registrados se encuentran cobrados.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Cliente / Organismo</TableHead>
              ...
              <TableBody>
                {collections.map((item) => {
                  const dias = getDaysRemaining(item.fecha_estimada_cobro);
                  const isVencido = dias !== null && dias < 0;

                  return (
                    <TableRow key={item.id}>
                      <TableCell className="font-mono text-xs text-slate-600 dark:text-slate-300">
                        {item.fecha_estimada_cobro ? formatDate(item.fecha_estimada_cobro) : '-'}
                      </TableCell>
                      ...
                      <TableCell className="text-xs font-semibold text-slate-900 dark:text-slate-100">
                        <div>
                          ...
                          <p className="text-[10px] font-normal text-slate-400">{item.direccion}</p>
                        </div>
                      </TableCell>
