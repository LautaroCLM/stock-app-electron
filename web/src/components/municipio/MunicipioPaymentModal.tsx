...
import { MunicipioOrder, MunicipioPayment } from '@/types/municipio';
...
interface MunicipioPaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const MunicipioPaymentModal: React.FC<MunicipioPaymentModalProps> = ({
  isOpen,
  onClose,
  order,
  onRefreshParent,
}) => {
  const [payments, setPayments] = useState<MunicipioPayment[]>([]);
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      setError(null);
      const data = await municipioWebService.getPayments(order.id);
      setPayments(data);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Error al cargar los cobros de la orden.';
      setError(msg);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (isOpen && order) {
      ...
      setMonto(order.saldo_pendiente ? String(order.saldo_pendiente) : '');
      setMetodoPago('Transferencia');
      setObservaciones('');
    }
  }, [isOpen, order, fetchPayments]);
  ...
  const handleAddPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(null);
      await municipioWebService.createPayment({
        orden_id: order.id,
  ...
  const handleDeletePayment = async (pagoId: number) => {
    if (!confirm('¿Eliminar este cobro registrado? El saldo de la orden será recalculado.')) return;

    try {
      setError(null);
      await municipioWebService.deletePayment(pagoId, order.id);
      await fetchPayments();
      onRefreshParent();
    } catch (err: unknown) {
  ...
  const saldoPendiente = Math.max(0, (order.total || 0) - totalPagado);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">
                Cobros de Orden Municipal #{order.id}
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {order.expediente ? `Expediente: ${order.expediente}` : order.orden_compra ? `Orden Compra: ${order.orden_compra}` : 'Gestión de pagos y saldos.'}
              </p>
            </div>
          </div>
        ...
        <div className="px-6 py-3 bg-slate-100/80 dark:bg-slate-800/40 border-b border-slate-200/60 dark:border-slate-800 grid grid-cols-3 gap-4 text-xs">
          <div>
            <span className="text-slate-500 dark:text-slate-400 block">Total Orden</span>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          <form onSubmit={handleAddPayment} className="p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-2xl space-y-3">
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Ej: Nro de transferencia 892341..."
                value={observaciones}
          ...
          <div>
            ...
            {isLoading ? (
              ...
              <div className="py-8 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl text-center">
                <p className="text-xs text-slate-400">No hay cobros registrados para esta orden municipal.</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
